...
<<label=get a dataframe and an interactome, eval=TRUE>>=
data(ER.limma)
data(hs.inter)
dt <- ER.limma
gi <- hs.inter
...
<<label=extract a subgraph and set attributes to RedeR, eval=TRUE>>=
...
<<label=extract another subgraph and set attributes to RedeR, eval=TRUE>>=
...
<<label=add subgraphs to the app, eval=TRUE>>=
...
<<label=nest subgraphs, eval=TRUE>>=
...
<<label=assign edges to containers, eval=TRUE>>=
mergeOutEdges(rdp)
@

\item 9 - relax the network
<<label=relax the network, eval=TRUE>>=
relax(rdp,50,400)
@

\item 10 - add a color legend (other types are available):
\footnote{Legends are add/set only via command line in the current version.}

<<label=add a color legend (other types are available), eval=TRUE>>=
...
<<label=select a gene, eval=TRUE>>=
selectNodes(rdp,"RET")
@

\item 12 - reset graph:
<<label=reset graph, eval=TRUE>>=
resetd(rdp)
@
...
<<label= add graph to the app, eval=TRUE>>=
addGraph(rdp,sg)
@

\item 5 - compute a hierarchical clustering using standard R functions:
<<label=compute a hierarchical clustering using standard R functions, eval=TRUE>>=
...
<<label=map the hclust object onto the network, eval=TRUE>>=
...
\item 9 - add color legend: 
\footnotemark[\value{footnote}]

<<label=add color legend, eval=TRUE>>=
...
\footnote{Due to build/check errors, this package was not available at the occasion of Bioconductor release; therefore, it had to be removed from RedeR dependencies. It should be installed by command line, as indicate in \emph{Installation} section.}
...
As usual with any new package, the developing process tends to be faster than the release schedule of R/Bioconductor. So, at this stage of the project we suggest using RedeR devel version available at \url{http://bioconductor.org/packages/devel/bioc/html/RedeR.html}.  Please download the \emph{Package Source} (the .tar.gz file!!) and install all dependences in R:
