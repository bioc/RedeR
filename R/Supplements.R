...
gtoy.rm <- function(
  ...
  if (!is.singleInteger(m) || m < 1) {
    stop("'m' must be an integer value > 0!")
  }
  if (!is.singleInteger(nmax)) {
    stop("'nmax' must be an integer value > 0!")
  }
  if (!is.singleInteger(nmin) || nmin >= nmax) {
    stop("'nmin' must be an integer value > nmax!")
  }
  if (!is.singleNumber(p1) || p1 >= 1 || p1 <= 0) {
    stop("'p1' must be numeric (0.0 < p < 1.0)!")
  }
  if (!is.singleNumber(p2) || p2 >= 1 || p2 <= 0) {
    stop("'p2' must be a numeric value (0.0 < p < 1.0)!")
  }
  if (!is.singleNumber(p3) || p3 >= 1 || p3 <= 0) {
    stop("'p3' must be a numeric value (0.0 < p < 1.0)!")
  }
  if (is.null(gg)) {
    par <- c(m = m, nmax = nmax, nmin = nmin, p1 = p1, 
        p2 = p2, p3 = p3)
...
.plot.signal <- function(gg, fname, isNew = TRUE) {
  dt <- data.frame(
    nodeId = V(gg)$name,
    signal = V(gg)$signal, moduleId = V(gg)$module.id
  )
  if (!is.null(fname)) {
  ...
  xlim <- c(0.5, m + 1.5)
  ylim <- c(0, gg$max)
  plot(0, xlim = xlim, ylim = ylim, ylab = "", xlab = "", 
      type = "n", axes = FALSE)
  if (!isNew) abline(ub, 0, lty = 2, col = "red", lw = 1.2)
  if (!isNew) abline(lb, 0, lty = 2, col = "red", lw = 1.2)
  boxplot(signal ~ moduleId,
    data = dt, col = cols,
    border = border, ylab = "", xlab = "", add = TRUE, axes = FALSE
  )
  axis(1,
    las = 1, labels = names(moduleId), at = moduleId,
    padj = 1, mgp = c(1, 0.1, 0), tcl = -0.3, lwd = 2.2, cex.axis = 1.2
  )
  axis(2, las = 2, mgp = c(0, 0.6, 0), tcl = -0.3, lwd = 2.2, 
      cex.axis = 1.2)
  mtext(text = "Simulated signal", side = 2, line = 2.2, cex = 1.3)
  mtext(text = "Simulated modules", side = 1, line = 2.2, 
      at = (m + 1) / 2, cex = 1.3)
  if (!isNew) {
    text(x = m + 1, y = ub + 0.03, "upper bound", cex = 0.7, 
        col = "red", adj = 0.5)
  }
  if (!isNew) {
    text(
      x = m + 0.5, y = (ub + lb) / 2, "random\nnodes",
      cex = 0.8, col = "red", adj = c(1, 0.5)
    )
  }
  if (!isNew) {
    text(x = m + 1, y = lb - 0.03, "lower bound", cex = 0.7, 
        col = "red", adj = 0.5)
  }
  if (!is.null(fname)) {
    dev.off()
...
.simulateModules <- function(par) {
  ...
  gg <- igraph::graph_from_adjacency_matrix(adj, mode = "undirected",
      diag = FALSE)
...
.simulateSignal <- function(gg) {
  ...
  signal <- runif(igraph::vcount(gg))
  sgdiff <- 2
  for (i in mods) {
    idx <- V(gg)$module.id == i
    md <- 1 + i * sgdiff - (sgdiff - 1)
    signal[idx] <- signal[idx] + rnorm(sum(idx), md)
  }
  signal <- signal - 1
  signal <- signal - min(signal)
  signal <- signal / max(signal)
  V(gg)$signal <- signal
  gg$max <- ceiling(max(V(gg)$signal))
  return(gg)
}
...
.setgg <- function(gg) {
  ...
  E(gg)$edgeColor <- "grey"
  E(gg)$edgeWidth <- 2
  cols <- c("darkblue", "blue", "cyan", "green", "yellow",
      "orange", "red", "darkred")
  breaks <- pretty(V(gg)$signal, n = 9)
  gg <- att.setv(
    g = gg, from = "signal", to = "nodeColor",
    cols = cols, breaks = breaks, pal = 1
  )
  V(gg)$signal.color <- V(gg)$nodeColor
  V(gg)$module.color <- terrain.colors(gg$par["m"])[V(gg)$module.id]
  V(gg)$nodeAlias <- as.character(V(gg)$module.id)
  class(gg) <- c(class(gg), "gtoy.rm")
  if (!is.null(gg$rid)) {
...
att.setv <- function(
    g = NULL, from = "name", to = "nodeColor", pal = 1, cols = NULL, 
    na.col = grey(0.7), xlim = c(20, 100, 1), shapes = NULL, breaks = NULL, 
    categvec = NULL, nquant = NULL, isrev = FALSE, getleg = TRUE,
    roundleg = 1, title = NULL) {
  # set att
  coltype <- c("nodeColor", "nodeLineColor", "nodeFontColor")
  numtype <- c(
    "nodeSize", "nodeLineWidth", "nodeFontSize",
    "nodeBend", "coordX", "coordY"
  )
  ...
  if (!is.character(from)) stop("arg. 'from' should be a string!")
  from <- from[1]
  if (!is.character(to) && !is.numeric(to) && !is.integer(to)) {
    stop("argument 'to' should be a string or an integer!")
  }
  to <- to[1]
  # get ref. att---
  fromatt <- igraph::vertex_attr(g, from)
  if (is.null(fromatt) || length(fromatt) != igraph::vcount(g)) {
    stop(paste("graph attribute '", from,
      "' is absent or not consistent with node count!",
      sep = ""
    ))
  }
  if (is.numeric(to) || is.integer(to)) {
    ...
    if (to < 1 || to > length(defaultatt)) {
      message(paste("Error: arg 'to=", to,
        "' is not consistent with available attr. options!",
        sep = ""
      ))
      message("*List of attributes handled by 'att.setv' function:")
      print(cbind(
  ...
  if (!to %in% defaultatt) {
    ...
    message("*List of attributes handled by 'att.setv' function:")
    print(cbind(attribute = defaultatt, id = 1:11, 
        description = description), quote = F
    )
    message("*Shape names and ids:")
    print(cbind(shape = defaultshapes, id = 1:5), quote = F)
    return(g)
  ...
  if (!is.null(breaks) && !is.numeric(breaks)) {
    stop("arg. 'breaks' should be numeric!")
  }
  if (!is.numeric(pal) && !is.integer(pal)) {
    stop("arg. 'pal' (pallete) should be an integer!")
  }
  pal <- pal[1]
  if (!pal %in% c(1, 2)) {
    stop("not a valid pallete (pal)! options: 1 or 2!")
  }
  if (!is.numeric(xlim) && !is.integer(xlim)) {
    stop("arg. 'xlim' should be numeric!")
  }
  if (length(xlim) < 3) {
    stop(paste("'xlim=", xlim,
      "' is not consistent with expected arg. length! ...3!",
      sep = ""
    ))
  }
  if (sum(is.null(xlim) > 0)) {
    stop("'xlim' arg. does not support null values!")
  }
  if (!is.logical(isrev)) {
    stop("'isrev' arg. should be a logical value!")
  }
  if (!is.null(nquant) && !is.numeric(nquant)) {
    stop("'nquant' arg. should be an integer!")
  }
  nquant <- nquant[1]
  ...
  colorcategory <- function(x, cols, na.col, categvec, isrev) {
    ...
    if (!is.null(categvec)) {
      if (sum(levels(x) %in% categvec) != length(levels(x))) {
        stop(paste0(
          "graph att. 'from' with one or more levels",
          "not represented in 'categvec' arg.!"
        ))
      }
  ...
  colorscale1 <- function(x, breaks, cols, na.col, isrev, 
      nquant, roundleg) {
    # check arg
    if (!is.null(nquant)) {
    ...
    if (sum(is.null(breaks)) > 0) {
      stop("breaks do not support null values!")
    }
    if (length(breaks) < 3) {
  ...
  colorscale2 <- function(x, breaks, cols, na.col, isrev, nquant, roundleg) {
    ...
    breaks <- format(breaks, digits = roundleg, nsmall = roundleg)
    leg <- list(scale = c(negCols, posCols), legend = breaks, 
        interval = interv)
    res <- list(res = x.col, leg = leg)
    return(res)
  }
  ...
  xcategory <- function(x, szmin, szmax, na.sz, categvec, isrev) {
    ...
    if (!is.null(categvec)) {
      if (sum(levels(x) %in% categvec) != length(levels(x))) {
        stop(paste0(
          "graph att. 'from' with one or more levels not ",
          "represented in 'categvec' arg.!"
        ))
      }
  ...
  xscale <- function(x, breaks, szmin, szmax, na.sz, xlim,
      isrev, nquant, roundleg) {
    # check arg
    if (!is.null(nquant)) {
      if (nquant < 2) {
  ...
  #--set node shapes for 'category' or 'enumerated' data type
  shapecategory <- function(x, shapes, categvec, isrev) {
    ...
    if (is.null(shapes)) {
      ...
      if (sum(sum(shapes < 1), sum(shapes > 5)) > 0) {
        msg <- paste0("Error: argument 'shapes' with one or more ",
          "inconsistent ids!")
        message(msg)
      ...
      if (sum(shapes %in% validnames) != length(shapes)) {
        msg <- paste0("Error: argument 'shapes' with one or more ",
          "inconsistent names!")
        message(msg)
        message("*List of shape names and ids handled by 
            'att.setv' function:")
        print(cbind(shape = validnames, id = 1:5), quote = F)
        return(g)
      }
    ...
    if (sum(is.na(x)) > 0) {
      stop("'NA' not supported for node shape mapping!")
    }
    if (isrev) shapes <- rev(shapes)
    x <- as.factor(x)
    if (length(levels(x)) > length(shapes)) {
      stop("graph att. 'from' with more levels than shape options!")
    }
    if (!is.null(categvec)) {
      if (sum(levels(x) %in% categvec) != length(levels(x))) {
        stop(paste0(
      ...
      if (length(categvec) != length(shapes)) {
        if (length(categvec) > length(shapes)) {
          stop("arg. 'categvec' with more levels than shapes!")
        } else {
  ...
  if (to %in% coltype) {
    ...
    if (is.null(breaks) && is.null(nquant)) {
      ...
      att <- xscale(fromatt, breaks, szmin, szmax, na.sz, 
          xlim, isrev, nquant, roundleg)
    }
...
att.sete <- function(
    g = NULL, from = "name", to = "edgeColor", pal = 1, cols = NULL, 
    na.col = grey(0.7), xlim = c(20, 100, 1), shapes = NULL, breaks = NULL, 
    categvec = NULL, nquant = NULL, isrev = FALSE, getleg = TRUE, 
    roundleg = 1, title = NULL) {
  # set att
  ...
  if (is.null(g)) {
    ...
    print(cbind(attribute = defaultatt, id = 1:5, description = description), 
        quote = F)
  ...
  from <- from[1]
  if (!is.character(to) && !is.numeric(to) && !is.integer(to)) {
    stop("arg. 'to' should be a string or an integer!")
  }
  to <- to[1]
  # get ref. att---
  ...
  if (is.null(fromatt) || length(fromatt) != igraph::ecount(g)) {
    stop(paste("graph attribute '", from,
      "' is absent or not consistent with edge count!",
      sep = ""
    ))
  }
  if (is.numeric(to) || is.integer(to)) {
    ...
    if (to < 1 || to > length(defaultatt)) {
      ...
      message("*List of attributes handled by 'att.sete' function:")
      print(cbind(attribute = defaultatt, id = 1:5, 
          description = description), quote = F
      )
      message("*Shape names and ids:")
  ...
  # main functions to set attribute scales!
  #----------------------------------------
  #--simple color palette for 'category' or 'enumerated' data type
  colorcategory <- function(x, cols, na.col, categvec, isrev) {
    if (is.null(cols)) 
        cols <- c("darkblue", "blue", "orange", "cyan", "red", "darkred")
    if (is.null(na.col)) {
  ...
  colorscale1 <- function(x, breaks, cols, na.col, isrev, nquant, roundleg) {
    ...
    if (!is.null(nquant)) {
      if (nquant < 2) stop("require at least two quantiles!")
      breaks <- quantile(x, probs = seq(0, 1, length.out = nquant), 
          na.rm = TRUE, names = FALSE)
      breaks <- unique(breaks)
      nquant <- length(breaks)
      if (length(breaks) < 3) stop("not enough intervals for 'nquant'!")
  ...
  #--neg/pos color scale palette with breaks (left/right)
  colorscale2 <- function(x, breaks, cols, na.col, 
      isrev, nquant, roundleg) {
    # check args
    if (!is.null(nquant)) {
      if (nquant < 2) stop("require at least two quantiles!")
    ...
    breaks <- format(breaks, digits = roundleg, nsmall = roundleg)
    leg <- list(scale = c(negCols, posCols), legend = 
            breaks, interval = interv)
    res <- list(res = x.col, leg = leg)
    return(res)
  }
  #--simple size scale for 'category' or 'enumerated' data type
  xcategory <- function(x, szmin, szmax, na.sz, categvec, isrev) {
    # set sz and return vec
    x <- as.factor(x)
    if (!is.null(categvec)) {
  ...
  #--size scale with breaks
  xscale <- function(x, breaks, szmin, szmax, na.sz, 
      isrev, nquant, roundleg) {
    # check arg
    if (!is.null(nquant)) {
      if (nquant < 2) stop("require at least two quantiles!")
  ...
  shapecategory <- function(x, shapes, categvec, isrev) {
    ...
    if (is.null(shapes)) {
      ...
      shapes <- as.integer(shapes)
      if (sum(sum(shapes < 1), sum(shapes > 5)) > 0) {
        message("Error: arg. 'edgeType' with one or more inconsistent ids!")
        message("*List of edgetype names  handled by the 'att.sete' function:")
      ...
      if (sum(shapes %in% validnames) != length(shapes)) {
        message("Error: arg. 'edgeType' with one or more inconsistent names!")
        message("*List of edgetype names handled by the 'att.sete' function:")
        print(cbind(shape = validnames, id = 1:4), quote = F)
        return(g)
      }
    ...
    if (sum(is.na(x)) > 0) {
      stop("'NA' not supported for edge type mapping!")
    }
    if (isrev) shapes <- rev(shapes)
    x <- as.factor(x)
    if (length(levels(x)) > length(shapes)) {
      stop("graph att. 'from' with more levels than edgetype options!")
    }
    if (!is.null(categvec)) {
      if (sum(levels(x) %in% categvec) != length(levels(x))) {
        stop(paste0(
      ...
      if (length(categvec) != length(shapes)) {
        if (length(categvec) > length(shapes)) {
          stop("arg. 'categvec' with more levels than edge types!")
        } else {
          shapes <- shapes[1:length(categvec)]
        }
      }
  ...
  if (to %in% coltype) {
    if (is.null(breaks) && is.null(nquant)) {
      ...
      if (pal == 1) {
        att <- colorscale1(fromatt, breaks, cols, na.col, isrev, 
            nquant, roundleg)
      } else if (pal == 2) {
        att <- colorscale2(fromatt, breaks, cols, na.col, isrev,
            nquant, roundleg)
      } else {
        att <- colorcategory(fromatt, cols, na.col, categvec, isrev)
      }
    ...
    if (is.null(breaks) && is.null(nquant)) {
      ...
      att <- xscale(fromatt, breaks, szmin, szmax, na.sz, isrev, 
          nquant, roundleg)
    }
  } else if (to == "edgeType") {
    att <- shapecategory(fromatt, shapes, categvec, isrev)
...
att.mapv <- function(g, dat, refcol = 1) {
  ...
  if (is.factor(dc)) {
    if (class(levels(dc)) == "character") {
      ...
      stop(paste0(
        "invalid ref. attribute! supported data classes: ",
        "character, numeric or integer!"
      ))
    }
  }
  ...
  if (class(nodes) == class(dc)) {
    ...
    stop("invalid node names! supported data classes: 
        character, numeric or integer!")
  }
  # check whether resulting dataset size matches nodes!
  if (nrow(dat) != length(nodes)) {
    stop("one or more graph nodes are not listed in 
        the dataset (i.e. 'refcol' ids)!")
  }
...
att.mape <- function(g, dat, refcol = c(1, 2)) {
  ...
  if (!all.integerValues(refcol)) {
    msg <- paste0("invalid 'refcol' value! it should be a two-column ",
      "index in 'data' object!")
    stop(msg)
  }
  ...
  # check whether resulting dataset size matches nodes!
  if (sum(is.na(idx1)) > 0) {
    msg <- paste0("one or more edges are not listed in the dataset ",
      "(in'refcol' ids)")
    stop(msg)
  }
  ...
  for (i in 1:ncol(dat)) {
    if (!i %in% refcol) {
      ...
      g <- igraph::set_edge_attr(graph = g, name = names(dat)[i],
          value = att)
    }
  }
  return(g)
...
subg <- function(
    g, dat, refcol = 1, maincomp = TRUE,
    connected = TRUE, transdat = TRUE) {
  if (is.data.frame(dat)) {
...
#-------------------------------------------------------------------------------
# function to generate random null distributions for co-expression analysis
cea <- function(x, sig = 0.01, padj.method = "fdr",
  ...
  if (!is.singleNumber(sig) || sig < 0 || sig > 1) {
    stop("'sig' should be a single numeric value in the interval [0,1]!")
  }
  if (!is.singleString(padj.method)) {
    stop("'padj.method' should be a single character value!")
  }
  if (!is.singleString(cor.method)) {
    stop("'cor.method' should be a single character value!")
  }
  if (!is.singleInteger(nper) || nper < 1) {
    stop("'nper' should be a single positive integer value!")
  }
  if (!is.null(regulators) && !is.character(regulators)) {
    stop("'regulators' should be a character vector!")
  }
  if (!is.singleLogical(plotcea)) {
    stop("'plotcea' should be a single logical value!")
  }
  if (is.null(rownames(x))) {
    stop("matrix 'x' should have rownames!")
  }
  if (!is.null(regulators)) {
    if (is.character(regulators)) {
      if (!all(regulators %in% rownames(x))) {
        stop("all 'regulators' should be listed in 'x' rownames!")
      }
  ...
  nulldist <- list()
  pb <- txtProgressBar(min = 0, max = nper, initial = 0, 
      char = "=", style = 1)
  ...
  rescea <- list(
    corr.mt = corrMt, decision.mt = decisionMt,
  ...
  ptcea <- function(rescea, ptype = 4, bk = 0.2, n.breaks = 100,
    ...
    if (ptype == 1) {
      plot(
        x = c(-1, 1), y = c(0, 1), col = "black", type = "n",
        main = "Gene co-expression analysis",
      ...
      #
      legend("topleft", c(
        "all associations", "sig. neg. associations",
        "sig. pos. association", "null distribution"
      ),
      pch = c(NA_integer_, 15, 15, 15),
      pt.cex = 1, lty = c(2, 0, 0, 0), merge = TRUE,
      col = c("black", "blue", "red", "grey"), bty = "n", cex = 0.7
      )
    }
    if (ptype == 2) {
      plot(
      ...
      if (plotnull) {
        ...
        if (avnull) {
          ...
          for (i in 1:min(100, ncol(x))) {
            lines(x = x[, i], y = y[, i], lwd = 1.0, col = i, lty = "dashed")
          }
        }
        legend("topleft", c(
          "all associations", "sig. neg. associations",
        ...
        pch = c(15, 15, 15, NA_integer_),
        pt.cex = 1, lty = c(0, 0, 0, 2), merge = TRUE,
        col = c("grey", "blue", "red", "black"), bty = "n", cex = 0.7
        )
      } else {
        legend("topleft", c(
          "all association", "sig. neg. associations",
          "sig. pos. associations"
        ),
        pch = c(15, 15, 15),
        pt.cex = 1, lty = c(0, 0, 0), col = c("grey", "blue", "red"),
        bty = "n", cex = 0.7
        )
      }
    }
    if (ptype == 3) {
      ...
      if (plotnull) {
        ...
        if (avnull) {
          ...
          for (i in 1:min(100, ncol(x))) {
            lines(x = x[, i], y = y[, i], lwd = 1.0, col = i, 
                lty = "dashed")
          }
        }
        legend("topleft", c(
    ...
    if (ptype == 5) {
      ...
      if (plotnull) {
        ...
        pch = c(15, NA_integer_, NA_integer_),
        pt.cex = 1, lty = c(0, 2, 2),
        col = c("grey", "blue", "red"), bty = "n", cex = 0.7
        )
      }
    }
  }
...
treemap <- function(hc) {
  ...
  xyaxis <- data.frame(xaxis = xaxis, yaxis = yaxis,
  ...
  N <- N[, 1] + N[, 2]
  obj <- list(
    nest = nest, compids = B, labels = hc$labels, parent = D, leafdist = C,
  ...
  #---get unified nodes
  hcl <- data.frame(
    node = hc$labels, mergeId = -c(1:nn), hcId = c(1:nn), type = "leaf",
    stringsAsFactors = FALSE
  )
  hcn <- data.frame(
    node = paste("N", c(1:N), sep = ""), mergeId = c(1:N), hcId = c(1:N),
    type = "nest", stringsAsFactors = FALSE
  )
  hcNodes <- rbind(hcl, hcn)
  hcEdges$parentNode <- hcNodes$node[
      match(hcEdges$parentNode, hcNodes$mergeId)]
  hcEdges$childNode <- hcNodes$node[
      match(hcEdges$childNode, hcNodes$mergeId)]
  #---update and return
  obj$hcNodes <- hcNodes
  obj$hcEdges <- hcEdges
  return(obj)
