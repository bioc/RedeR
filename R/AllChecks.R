#-------------------------------------------------------------------------------
.validate.old.args <- function(name, para) {
    if (name == "theme") {
        para <- gsub("tm", "th", para)
    }
    if (name == "att.setv") {
        para <- ifelse(para == "nodeAlias", "nodeLabel", para)
        para <- ifelse(para == "nodeFontSize", "nodeLabelSize", para)
...
.validate.args <- function(check, name, para) {
    if (check == "numeric_vec") {
        ...
        if (!is.vector(para) || !.all_numericValues(para))
            stop(msg, call. = FALSE)
        ...
        msg <- paste0("'", name, "' should be a vector with strings.")
        if (!.all_characterValues(para, notNA = FALSE)) 
            stop(msg, call. = FALSE)
    ...
    } else if (check == "singleStringOrNumber") {
        msg <- paste0("'", name,
            "' should be a single string or numeric value.")
        if (!.is_singleNumber(para) && !.is_singleString(para))
            stop(msg, call. = FALSE)
    }
}
